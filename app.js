...
function resetPractice() {
  practiceCards = shuffle(scopedCards());
  practiceIndex = 0;
  renderPractice();
}
...
function renderPractice() {
  ...
  $("practiceMeta").textContent = `${practiceIndex + 1} van ${practiceCards.length} - ${card.section}`;
...
$("practiceShowBtn").addEventListener("click", () => $("practiceAnswer").classList.remove("hidden"));
$("practiceCard").addEventListener("click", () => { if ($("practiceMode").value === "card") $("practiceAnswer").classList.remove("hidden"); });
$("nextPracticeBtn").addEventListener("click", () => { if (practiceCards.length) practiceIndex = (practiceIndex + 1) % practiceCards.length; renderPractice(); });
$("prevPracticeBtn").addEventListener("click", () => { if (practiceCards.length) practiceIndex = (practiceIndex - 1 + practiceCards.length) % practiceCards.length; renderPractice(); });
...
document.addEventListener("keydown", (event) => {
  ...
  if (activeView === "practice") {
    if (event.key === " " && $("practiceMode").value === "card") { event.preventDefault(); $("practiceAnswer").classList.remove("hidden"); }
    if (event.key === "ArrowRight") $("nextPracticeBtn").click();
    if (event.key === "ArrowLeft") $("prevPracticeBtn").click();
  }
});
